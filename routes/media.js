...
router.get('/bed/:id/media/', jsonParser, (req, res) => {
...
router.post('/bed/:id/media/', jsonParser, (req, res) => {
  ...
  const media = new Media({ "message": req.body.message });
  //console.log(`Adding notification to bed \`${req.body.bednumber}\``);
  Gardenbeds.findOneAndUpdate({ "bedNumber": req.params.id }, { $push: { "media": media } },
...
router.delete('/bed/:bedNumber/media/:id', jsonParser, (req,res) => {
  Gardenbeds.update({"bedNumber":req.params.bedNumber}, { $pull: {"media":{"_id":
  req.params.id}}},
    function(error,success) {
      if (error) {
        //console.log("ERROR: " + error);
